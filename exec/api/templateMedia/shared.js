...
export const TemplatesMedia = new FilesCollection({
  collectionName: 'templatesMedia',
  storagePath : Meteor.settings.public.storagePath+"/TemplatesMedia", // The patch where files are inserted
...
Meteor.methods({
  "templatesMedia.delete"(_id){
    try {
      assert(!!_id, "No _id received for delete.")
      const del = TemplatesMedia.remove({_id})
      return !!del
    } catch (error) {
  ...
  "templatesMedia.update"(item){
    try {
      ...
      const upd = TemplatesMedia.update(
        {
          _id
        },
        {
